...
BalanceCache.prototype = {
  ...
  /**
   * increment By Float. If key doesn't exist value is set
   *
   * @param {string} owner - address of user whose balance is to be set
   * @param {bigNumber} byValue - +ve or -ve bignumber value. e.g. "-100000000000000000000" or "2000000000000000000000000"
   *
   * @return {promise<result>}
   *
   */
  incrementByFloat: function(owner, byValue) {
    const oThis = this
      , cacheKey = cacheKeys.btBalance(oThis.chainId, oThis.brandedTokenAddress, owner)
    ;

    return cacheImplementer.incrementByFloat(cacheKey, byValue.toString(10));
  }
