 ...
 * @param {Bool} useCache - get from cache or from db
 *
 * @return {Object}
 *
 */
const AirdropUserBalanceKlass = function(params) {
  ...
  oThis.useCache = params.useCache;
...
AirdropUserBalanceKlass.prototype = {
  ...
  perform: async function () {
    ...
    if (oThis.useCache){
      logger.debug("=======perform.fromCache=======");
      r = await oThis.getUserAirdropBalance();
    } else {
      logger.debug("=======perform.fromDb=======");
      const userAirdropDetailModel = new UserAirdropDetailKlass();
      r = await userAirdropDetailModel.getByUserAddresses(oThis.airdropRecord.id, oThis.userAddresses);
    }
