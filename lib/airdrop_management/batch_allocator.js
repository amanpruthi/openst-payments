...
const batchAllocator = module.exports = function(params) {
  ...
  oThis.userAddresses = [];
  oThis.existingUserAirdropDetail = {};
};
...
batchAllocator.prototype = {
  ...
  validateParams: function() {
    ...
    return new Promise(async function (onResolve, onReject) {
      ...
      oThis.setExistingUserAirdropData();
      for (var userAddress in oThis.airdropUsers) {
        const value = oThis.airdropUsers[userAddress];
        ...
        const userAirdropAmount = new BigNumber(value.airdropAmount);
        ...
        const expiryTimestamp = new BigNumber(value.expiryTimestamp);
        ...
        // If existing user
        var existingUser = oThis.existingUserAirdropDetail[userAddress];
        if (existingUser){
          oThis.updateData.push(
            { id: existingUser.id,
              airdropAmount: userAirdropAmount.toString(10),
          ...
          oThis.bulkInsertData.push(data);
        }
      }
  ...
  /**
   * Get getExistingUserAirdropDetails
   *
   * @return {Promise}
   *
   */
  setExistingUserAirdropData: async function(){
    // Collect all user addresses
    for (var userAddress in oThis.airdropUsers) {
      oThis.userAddresses.push(userAddress);
    }
    var userAirdropDetailModel = new userAirdropDetailKlass();
    const result = await userAirdropDetailModel.select("id, user_address").where({airdrop_id: oThis.airdropRecord.id, user_address: oThis.userAddresses}).fire();
    for (var uadIndex in result) {
      var userAirdropDetail = result[uadIndex];
      oThis.existingUserAirdropDetail[userAirdropDetail.user_address] = {
        id: userAirdropDetail.id
      };
    }
  },
  ...
  allocateAirdropAmountToUsers: async function() {
    ...
    return new Promise(async function (onResolve, onReject) {
      try {
        ...
        // Update one by one in loop
        for (var index in oThis.updateData) {
          const updateData = oThis.updateData[index];
          await userAirdropDetailModel.update(["airdrop_amount=airdrop_amount+?", updateData.airdropAmount]).where({id: updateData.id}).fire();
        }
