...
BrandedToken.prototype = {
  ...
  creditBalance: async function (owner, bigAmount) {
    ...
    try {
      ...
      if (bigAmount.gt(0)) {
        const incrementByFloatResponse = await oThis.balanceCache.incrementByFloat(owner, bigAmount);
        logger.info("=========creditBalance.incrementByFloat======", incrementByFloatResponse);
        if (incrementByFloatResponse.isSuccess() && incrementByFloatResponse.data.response != null) {
          return Promise.resolve(responseHelper.successWithData({}));
        }
        return Promise.resolve(responseHelper.error('l_ci_bt_creditBalance_1', incrementByFloatResponse));
      }
  ...
  debitBalance: async function (owner, bigAmount) {
    ...
    try {
      bigAmount = basicHelper.convertToBigNumber(bigAmount);
      if (bigAmount.gt(0)) {
        // Multiplied by minus 1 needed for subtraction as per redis syntax
        const incrementByFloatResponse = await oThis.balanceCache.incrementByFloat(owner, bigAmount.mul(-1));
        logger.info("=========debitBalance.incrementByFloat======", incrementByFloatResponse);
        if (incrementByFloatResponse.isSuccess() && incrementByFloatResponse.data.response != null) {
          return Promise.resolve(responseHelper.successWithData({}));
        }
        return Promise.resolve(responseHelper.error('l_ci_bt_debitBalance_1', incrementByFloatResponse));
      }
